...
\begin{titlepage}
    ...
    {\Huge \textbf{E-Commerce Apple Product}}\\[0.2cm]
...
The complete source code is publicly available at \url{https://github.com/Thanh36-jqk/Project3}, and a live demonstration is accessible at \url{https://project3-icy1.onrender.com/}.
...
\begin{enumerate}
    \item \textbf{To architect a scalable and secure backend infrastructure} utilizing \textbf{Node.js} and \textbf{Express.js} framework, implementing RESTful API design principles with JWT-based stateless authentication to enable horizontal scalability.
    
    \item \textbf{To design and implement a robust database schema} using \textbf{MongoDB} (NoSQL) with proper indexing strategies, ensuring data integrity through validation middleware and Mongoose schema constraints. The schema shall normalize data to at least \textbf{Third Normal Form (3NF)} equivalent where applicable to relational concepts.
    
    \item \textbf{To develop a modern, responsive frontend} using semantic \textbf{HTML5}, modular \textbf{CSS3} (with Tailwind utility framework), and vanilla \textbf{JavaScript} (ES6+), ensuring cross-browser compatibility and mobile-first design philosophy.
...
This chapter provides a comprehensive survey of the core technologies, frameworks, and libraries employed in the system, accompanied by comparative analyses that justify each selection against viable alternatives. The discussion is structured around the classical three-tier architecture: \textbf{Presentation Layer} (Frontend), \textbf{Application Layer} (Backend/API), and \textbf{Data Layer} (Database).

\section{Related Work and Literature Review}

Before detailing the technology stack, it is essential to position this work within the broader context of e-commerce system development and related academic research.

\subsection{Academic Research on E-Commerce Architectures}

Recent literature has extensively explored architectural patterns for scalable web applications:

\begin{itemize}
    \item \textbf{Microservices for E-Commerce:} Richardson's work on microservices architecture demonstrates the benefits of decomposing monolithic e-commerce platforms into independent services for improved scalability and team autonomy. However, this approach introduces operational complexity (service mesh, distributed tracing) unsuitable for academic projects with limited DevOps resources.
    
    \item \textbf{NoSQL Database Selection:} Comparative studies on MongoDB vs. PostgreSQL for e-commerce workloads show that document databases excel in read-heavy scenarios with flexible schemas, while relational databases provide stronger ACID guarantees for financial transactions. This project leverages MongoDB's schema flexibility for product catalog diversity while acknowledging transactional limitations.
    
    \item \textbf{JWT vs. Session-Based Authentication:} Security research comparing stateless JWT tokens with traditional session cookies highlights trade-offs: JWTs enable horizontal scaling (no shared session store) but cannot be invalidated server-side without additional infrastructure (Redis blacklist). This work implements JWT for API authentication combined with OAuth for social login.
\end{itemize}

\subsection{Existing E-Commerce Platforms}

Commercial platforms provide benchmarks for feature comparison:

\begin{table}[H]
\centering
\caption{Comparison with Existing E-Commerce Solutions}
\label{tab:related_work_comparison}
\small
\begin{tabular}{@{}p{0.20\linewidth}p{0.25\linewidth}p{0.25\linewidth}p{0.20\linewidth}@{}}
\toprule
\textbf{Platform} & \textbf{Technology Stack} & \textbf{Customization} & \textbf{Target Users} \\
\midrule
\textbf{Shopify} & Ruby on Rails, Liquid templating & Limited (plugin-based) & Non-technical merchants \\
\textbf{WooCommerce} & PHP, WordPress, MySQL & Moderate (hooks/filters) & WordPress users \\
\textbf{Magento} & PHP, Zend Framework & High (complex codebase) & Enterprise retailers \\
\textbf{This Project} & Node.js, MongoDB, Vanilla JS & Full (open source code) & Developers, learners \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key Differentiators of This Work:}
\begin{enumerate}
    \item \textbf{Modern JavaScript Stack:} Unlike legacy PHP-based platforms (WooCommerce, Magento), this project employs a unified JavaScript ecosystem, enabling code reuse between frontend and backend (e.g., validation schemas).
    
    \item \textbf{Native AI Integration:} While Shopify and WooCommerce require third-party plugins for chatbot functionality, this system integrates Google Gemini API directly into the codebase with context-aware product catalog access.
    
    \item \textbf{3D Product Visualization:} Three.js WebGL rendering provides interactive 3D models without browser plugin requirements, a feature absent in most open-source e-commerce platforms.
    
    \item \textbf{Academic Documentation:} Commercial platforms prioritize user guides over technical documentation. This thesis provides detailed architecture diagrams, API specifications, and testing methodologies valuable for educational purposes.
\end{enumerate}

\subsection{Research Gap Addressed}

Existing academic literature and commercial platforms exhibit the following gaps:

\begin{itemize}
    \item \textbf{Lack of Full-Stack Case Studies:} Most computer science curricula teach backend and frontend in isolation. This project demonstrates end-to-end integration (database $\rightarrow$ API $\rightarrow$ UI) with production-grade practices (JWT, CORS, input validation).
    
    \item \textbf{Limited AI Chatbot Integration Examples:} While Large Language Models (LLMs) like GPT and Gemini are widely discussed, practical examples of integrating AI into domain-specific applications (e-commerce product queries) with contextual data (user order history, inventory status) are scarce in academic repositories.
    
    \item \textbf{Performance Benchmarking Gap:} Many student projects lack rigorous performance testing. This work includes Apache JMeter load tests, OWASP ZAP security audits, and SUS usability studies, bridging the gap between academic prototypes and production systems.
\end{itemize}

The subsequent sections detail the technology choices that address these gaps while maintaining feasibility within undergraduate project constraints.
